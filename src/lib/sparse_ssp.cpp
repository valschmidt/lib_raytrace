...
vector< vector<double> > sparsed_ssp::raytrace(double z0, double x0, double theta0, double owtt, double max_gradient)
{
    ...
    for (int i = 0; i < gradient.size() - 1; i++)
    {
        ...
        else if (fabs(gradient[i] - gradient[i + 1]) <= max_gradient)
        {
